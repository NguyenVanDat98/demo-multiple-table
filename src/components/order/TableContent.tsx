  ...
  const column: TableColumnsType<ProductBase> = [
    {
      ...
      render: (id, r, i) => {
        ...
        return (
          <Image.PreviewGroup items={r.images}>
        ...
        return (
          ...
          >
            {() => {
              ...
              const { price } = r.variants.find(
                ({ variantIsDefault: e, _id }) =>
    ...
    },
    {
      ...
      render: (v, r, i) => {
        ...
        const { exchangeValue } = r.variants.find(({ isDefault: e, _id }) =>
          variantId ? String(_id) === String(variantId) : e
        ) ?? { exchangeValue: 1 };
        ...
        return (
          <InputNumberAlign
            ...
            parser={(val) => {
              return Number(String(val).replace(/[.,]/gi, "")) * exchangeValue;
            }}
            formatter={(v) => {
    ...
    },
    {
      ...
      width: 170,
      align: "left",
      render: (record: Variant[], _r, i) => {
        const variants = record.map(
          ({
            _id: value,
            unit: { name: label },
            variantIsDefault: isDefault,
          }) => ({
            value,
            label,
            isDefault,
        ...
        if (record.length > 1) {
          ...
          return (
            <div style={{ paddingInline: 10 }}>{variantDefaut?.label}</div>
          );
        }
      },
    },
      ...
      render: (id, _r, i) => {
        return (
          <Button
            onClick={() => {
              remove(i)
              setSeletedProduct((acc)=>{
                  return acc.filter(({_id})=>_id!==id)
              })
            }}
            danger
            icon={<DeleteOutlined />}
